...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnModeDynamic && btnModeStatic) {
    btnModeDynamic.addEventListener('click', () => {
      mode = 'dynamic';
      btnModeDynamic.classList.add('bg-emerald-500/20', 'text-emerald-400', 'border-emerald-500/40');
      btnModeDynamic.classList.remove('text-slate-400', 'border-white/10');
      btnModeStatic.classList.remove('bg-emerald-500/20', 'text-emerald-400', 'border-emerald-500/40');
      btnModeStatic.classList.add('text-slate-400', 'border-white/10');
    });
    btnModeStatic.addEventListener('click', () => {
      mode = 'static';
      carX = canvas.width / 2 - 80;
      btnModeStatic.classList.add('bg-emerald-500/20', 'text-emerald-400', 'border-emerald-500/40');
      btnModeStatic.classList.remove('text-slate-400', 'border-white/10');
      btnModeDynamic.classList.remove('bg-emerald-500/20', 'text-emerald-400', 'border-emerald-500/40');
      btnModeDynamic.classList.add('text-slate-400', 'border-white/10');
    });
  }
  ...
  function animate() {
    ...
    for (let i = 0; i < numCoils; i++) {
      ...
      if (inRange) {
        ...
        ctx.strokeStyle = `rgba(16, 185, 129, ${0.45 + Math.sin(fluxPulse) * 0.25})`;
        ctx.lineWidth = 2;
        ctx.setLineDash([4, 4]);
    ...
    ctx.strokeStyle = isCharging ? '#10B981' : '#38BDF8';
    ctx.lineWidth = 2;
    if (isCharging) {
      ctx.shadowColor = '#10B981';
      ctx.shadowBlur = 12;
    ...
    ctx.fillStyle = isCharging ? '#10B981' : '#0284C7';
    ...
    if (activeCoilDisplay) {
      ...
      activeCoilDisplay.className = isCharging ? 'text-emerald-400 font-semibold' : 'text-slate-400 font-normal';
    }
  }
